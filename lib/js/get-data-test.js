...
client
  ...
  .then(function (ob) {
    ...
    if (weight != null && weight.length > 1) {
      ...
      if (weight[0].note != null && weight[0].note.length > 1) {
        weight[0].note.sort((a, b) => {
          return new Date(b.time) - new Date(a.time);
        });
        displayAnnotation(weight[0].note[0].text);
      } else if (weight[0].note != null && weight[0].note.length == 1) {
        displayAnnotation(weight[0].note[0].text);
      }
...
// var medResults = [
//   "SAMPLE Lasix 40mg",
//   "SAMPLE Naproxen sodium 220 MG Oral Tablet",
//   "SAMPLE Amoxicillin 250 MG",
// ];
...
client
  ...
  .then(function (pat) {
    if (
      ...
      displayMedication("No medications for patient id: " + client.patient.id);
    }
